...
export class EmpSideNavbarComponent implements OnInit {
 ...
 ngOnInit(): void {
  ...
  if (user) {
    ...
    // Filter nav items based on email
    this.navItems = this.getFilteredNavItems(user.email);
  } else {
...
getFilteredNavItems(email: string) {
  const baseItems = [
    {
      label: 'My Profile',
      route: '/profile',
  ...
  const AssetManagerOnlyItems = [
    {
      label: 'Asset Management',
      route: '../hr/asset-management-hr',
      icon: 'M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2H5a2 2 0 00-2 2zm3 3a1 1 0 011-1h1a1 1 0 010 2H7a1 1 0 01-1-1zm4-1a1 1 0 000 2h4a1 1 0 100-2h-4z'
    }
  ];

  if (email === 'arjun@kaizenque.com') {
    return [...baseItems, ...AssetManagerOnlyItems];
  }
